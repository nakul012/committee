...
from auth_app.common.choices import USER_TYPE
...
class MyUserManager(BaseUserManager):
    ...
    def create_superuser(self, email, password=None):
        ...
        return:
            user (MyUserManager): user object
        """
        user = self.create_user(email, password=password, user_type="S")
        user.user_type = "S"
        user.is_admin = True
        user.save(using=self._db)
        return user
...
class Account(AbstractBaseUser, AbstractTime):
    ...
    user_type = models.CharField(
        "User Type", choices=USER_TYPE, max_length=250, null=False, blank=False
    )
